...
const resolveUploadPath = (...segments) => {
    ...
    if (resolved === UPLOAD_ROOT_RESOLVED || resolved.startsWith(`${UPLOAD_ROOT_RESOLVED}${path.sep}`)) {
        return resolved;
    }
...
const storage = multer.diskStorage({
    destination: (req, _file, cb) => {
        ...
        const targetDir = resolveUploadPath(userSegment, year, month, day);
    ...
    try {
        ...
        try {
            ...
            if (convertedImagePath) {
                const convertedFullPath = resolveUploadPath(convertedImagePath);
                fs.unlink(convertedFullPath, (unlinkError) => {
                    if (unlinkError) {
                        logger.warn('Failed to cleanup converted upload image', {
        ...
        if (convertedImagePath) {
            filesToDelete.push(resolveUploadPath(convertedImagePath));
        }

        filesToDelete.forEach(filePath => {
            fs.unlink(filePath, (unlinkError) => {
                if (unlinkError) {
                    logger.warn('Failed to cleanup upload file', { path: filePath, error: unlinkError.message });
                }
            });
        });
...
router.get('/:uploadId/file', async (req, res) => {
    try {
        ...
        const filePath = resolveUploadPath(uploadRow.stored_filename);
...
router.get('/:uploadId/converted', async (req, res) => {
    try {
        ...
        const filePath = resolveUploadPath(uploadRow.converted_image_path);
    ...
    try {
        ...
        try {
            ...
            const fileDescriptors = [
                { path: resolveUploadPath(stored_filename), type: 'original' }
            ];

            if (is_dicom && converted_image_path) {
                fileDescriptors.push({
                    path: resolveUploadPath(converted_image_path),
                    type: 'converted'
                });
            }
